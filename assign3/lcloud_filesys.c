...
//Counter to keep track of what device we are on before knowing the ID
int device_counter = 5;
...
int lcwrite( LcFHandle fh, char *buf, size_t len ) {
    ...
    int i, f, blocks, j, p, z, emptydev, emptyblock, emptysector, block, sector, dev;
    ...
    while (templen > 0){
        ...
        /*
        if (fullcount >= devicecount && refilled >0 && totalempty == refilled){
            logMessage(LcDriverLLevel, "\n\n FUCK YOOUU\n\n");
            if (templen>256){
                            block = instancearray[f].blocklist[newblk];
                            sector = instancearray[f].sectorlist[newblk];
                            dev = instancearray[f].devicelist[newblk];
                            ...
                            instancearray[f].devicelist[instancearray[f].writecount] = dev;
                            instancearray[f].sectorlist[instancearray[f].writecount] = sector;
                            instancearray[f].blocklist[instancearray[f].writecount] = block;

                            lcloud_putcache(dev, sector, block, locbuf);
                            writeblock(dev, locbuf, sector, block);
                            memset(locbuf, 0, 256);
        ...
        while (instancearray[f].writepos[z] != 0){
            ...
            if (foundpartial == 1){
                ...
                if (templen > (256 - instancearray[f].writepos[p]) && full == 0){
                    ...
                    /*
                    //Manually put each byte into the malloc array
                    for (int j = 0; j<(256 - instancearray[f].writepos[p]); j++){
                        memarr[j] = buf[transfer];
                        transfer+=1;
                    }

                    //memcpy the necessary amount to fill the remainder of the block
                    memcpy(&locbuf[instancearray[f].writepos[p]], memarr, (256 - instancearray[f].writepos[p]));
                    */
                ...
                if (templen <= (256 - instancearray[f].writepos[p]) && full == 0){
                    ...
                    //memcpy the necessary amount to fill the remainder of the block
                    memcpy(&locbuf[instancearray[f].writepos[p]], memarr, templen);
                    */
...
int lcclose( LcFHandle fh ) {
    ...
    for (j=0;j<instancearray[f].writecount;j++){
        ...
        //Allow us to rewrite to this block later
        for(int d = 0; d < devicecount;d++){
...
int *readblock(int devid, char *buf, int sector, int block){
    ...
    //if ( (frm== -1) || ((frm= lcloud_io_bus(frm, buf)) == -1) ||(b0 != 1) || (b1 != 1) || (c0 != LC_BLOCK_XFER) ) {
